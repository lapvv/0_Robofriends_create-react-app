...
const PostForm = ({create}) => {
  ...
  const addNewPost =(event)=>{
    ...
    return (
        ...
        <MyInput type='text' placeholder='Post header' value={post.title} onChange={event=>setPost({...post, title:event.target.value})}/>
        <MyInput type='text' placeholder='Post content' value={post.body} onChange={event=>setPost({...post, body:event.target.value})}/>
        <MyButton onClick={addNewPost} style={{color: "black"}}>Create</MyButton>
        {/* <MyButton style={{color: "black"}}>Clear</MyButton> */}
      </form>
    );
};
